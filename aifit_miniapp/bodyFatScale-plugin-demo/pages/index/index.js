...
Page({
  ...
  onShow: function () {
    ...
    var scaleFatData = plugin.getBodyScaleDataByInet(
      170,
      60,
      1,
      fatData.bfr,
  ...
  onLoad: function (options) {
    ...
    that.longnv();
    if (
      wx.getStorageSync("userInfo").avatarUrl == null ||
      wx.getStorageSync("userInfo").nickName == null
    ) {
      that.setData({
        ...
        wx.openBluetoothAdapter({
          success: function (res) {
            ...
            that.setData({
              linksToPrompt:
                "已绑定广播称:" +
                wx.getStorageSync("devMacAddress") +
                "，请上秤",
            });
            //  保持监测
            //   蓝牙已开启  开始
          ...
          success: function (res) {
            wx.createBLEConnection({
              ...
              success: function (res) {
                ...
                that.setData({
                  linksToPrompt:
                    "已连接" + wx.getStorageSync("devMacAddress") + ",请上秤",
                });
                that.link_device();
              },
              fail: function (res) {
  ...
  onBluetoothDeviceFound() {
    wx.onBluetoothDeviceFound((res) => {
      res.devices.forEach((device) => {
        if (
          device.advertisServiceUUIDs == null ||
          device.advertisServiceUUIDs.length == 0
        ) {
          //判断是否为15设备
          if (
            deviceInfo != "1" &&
            deviceInfo != null &&
            deviceInfo.macAddress.length != 9
          ) {
            //判断是否已经绑定设备
            var networkData = plugin.getBbroadcastData(device.advertisData); //获取广播称基础数据
            if (
              networkData != null &&
              networkData.address == deviceInfo_address
            ) {
              //判断基础数据是否为空，广播数据的mac地址是否等于绑定设备mac地址
              var sex = obtainSex(this.data.sex); //获取用户性别
              var age = obtainAge(this.data.briday); //获取用户年龄、
              var heigjtObt = obtainHeight();
              console.log(
                "sex:" + sex + ";age:" + age + ";height：" + heigjtObt
              );
              if (networkData.cmdType == 3) {
                //称重数据稳定，阻抗测量结束
                if (networkData.adc > 0) {
                  //有阻抗时
                  var fatData = plugin.getBodyDataByInet(
                    age,
                    heigjtObt,
                    sex,
                    networkData.weight,
                  ...
                  var weightStandard = standardTool.getWeightDisplay(
                    heigjtObt,
                    networkData.weight
                  ); //获取体重判定标准等级
