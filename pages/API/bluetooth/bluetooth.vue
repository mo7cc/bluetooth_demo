<template>
  <view>
    ...
    <view class="uni-padding-wrap uni-common-mt">
      <view>
        本蓝牙协议只支持低功耗蓝牙协议ble。如果想连接非ble蓝牙设备，请在社区搜索
        Native.js 蓝牙。
      </view>
      ...
      >
        ...
        >
          ...
          <view v-if="showMaskType === 'characteristics'">
            ...
            <view class="uni-list_item"
              >是否支持 read 操作:{{ item.properties.read }}</view
            >
            <view class="uni-list_item">
      ...
      uni.getBluetoothDevices({
        success: (res) => {
          ...
          if (res.errMsg === "getBluetoothDevices:ok") {
            this.getDevicesListData(res.devices);
    ...
    getDevicesListData(list) {
      for (let i = 0; i < list.length; i++) {
        ...
        if (
          ...
          console.log(`mo7-${i}`, ab2hex(list[i].advertisData));
        } else {
          console.log("不是我们的秤");
        }
      }
